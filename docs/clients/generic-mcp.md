...
2. Start orchestration: Luna/max selects a fixed bundle or one profile; Terra/medium executes roles one at a time in a fixed order, and the host passes `completed_profile` after each role. Keep `speed=1.0` for every model stage. Optional Sol/high and Terra/medium synthesis are available only after the final role.
